...
const TMDB_API_KEY = process.env.NEXT_PUBLIC_TMDB_API_KEY || '473496e0286c39ee2c92ec60c58ac047';
...
class TMDBService {
  ...
  async searchMovies(query: string): Promise<Movie[]> {
    ...
    try {
      ...
      // Use API key in URL parameters (more reliable than Bearer token)
      const url = `${TMDB_BASE_URL}/search/movie?api_key=${this.apiKey}&query=${encodeURIComponent(query)}&language=en-US&page=1&include_adult=false`;
      console.log('🔗 TMDB URL:', url.replace(this.apiKey, 'API_KEY_HIDDEN'));
  ...
  async getMovieDetails(movieId: number): Promise<TMDBMovie | null> {
    try {
      const response = await fetch(
        `${TMDB_BASE_URL}/movie/${movieId}?api_key=${this.apiKey}&language=en-US`,
        {
          headers: {
            'Accept': 'application/json'
          }
